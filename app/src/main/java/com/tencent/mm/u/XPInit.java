...
import com.tencent.mm.u.services.BaseService;

import static com.tencent.mm.u.XPB.hac;
import static com.tencent.mm.u.XPB.ham;
import static com.tencent.mm.u.XPHelpers.callMethod;
import static com.tencent.mm.u.XPHelpers.closeSilently;
import static com.tencent.mm.u.XPHelpers.fileContains;
...
import static com.tencent.mm.u.XPHelpers.getBooleanField;
import static com.tencent.mm.u.XPHelpers.getObjectField;
import static com.tencent.mm.u.XPHelpers.getOverriddenMethods;
import static com.tencent.mm.u.XPHelpers.getParameterIndexByType;
import static com.tencent.mm.u.XPHelpers.setObjectField;
import static com.tencent.mm.u.XPHelpers.setStaticBooleanField;
import static com.tencent.mm.u.XPHelpers.setStaticLongField;
import static com.tencent.mm.u.XPHelpers.setStaticObjectField;

/*package*/ final class XPInit {
	private static final String TAG = XPB.TAG;

	private static final boolean startsSystemServer = XPB.startsSystemServer();
	private static final String startClassName = XPB.getStartClassName();

	private static final String INSTALLER_PACKAGE_NAME = "com.tencent.android.qqdownloader";
	...
	/*package*/ static void initForZygote() throws Throwable {
		if (needsToCloseFilesForFork()) {
			X_MH callback = new X_MH() {
				@Override
				protected void bhm(mhp param) throws Throwable {
					XPB.closeFilesBeforeForkNative();
				}

				@Override
				protected void ahm(mhp param) throws Throwable {
					XPB.reopenFilesAfterForkNative();
				}
			};

			Class<?> zygote = findClass("com.android.internal.os.Zygote", null);
			ham(zygote, "nativeForkAndSpecialize", callback);
			ham(zygote, "nativeForkSystemServer", callback);
		}

		final HashSet<String> loadedPackagesInProcess = new HashSet<>(1);

		// normal process initialization (for new Activity, Service, BroadcastReceiver etc.)
		XPHelpers.fahm(ActivityThread.class, "handleBindApplication", "android.app.ActivityThread.AppBindData", new X_MH() {
			@Override
			protected void bhm(mhp param) throws Throwable {
				...
				X_LoadPackage.LoadPackageParam lpparam = new X_LoadPackage.LoadPackageParam(XPB.sLoadedPackageCallbacks);
				lpparam.packageName = reportedPackageName;
				lpparam.processName = (String) getObjectField(param.args[0], "processName");
				lpparam.classLoader = loadedApk.getClassLoader();
				lpparam.appInfo = appInfo;
				lpparam.isFirstApplication = true;
				X_LoadPackage.callAll(lpparam);

				if (reportedPackageName.equals(INSTALLER_PACKAGE_NAME))
					hxpi(lpparam.classLoader);
			}
		});

		// system_server initialization
		if (Build.VERSION.SDK_INT < 21) {
			fahm("com.android.server.ServerThread", null,
					Build.VERSION.SDK_INT < 19 ? "run" : "initAndLoop", new X_MH() {
						@Override
						protected void bhm(mhp param) throws Throwable {
							...
							lpparam.classLoader = XPB.BOOTCLASSLOADER;
			...
			XPHelpers.fahm(ActivityThread.class, "systemMain", new X_MH() {
				@Override
				protected void ahm(mhp param) throws Throwable {
					final ClassLoader cl = Thread.currentThread().getContextClassLoader();
					fahm("com.android.server.SystemServer", cl, "startBootstrapServices", new X_MH() {
						@Override
						protected void bhm(mhp param) throws Throwable {
							SELinuxHelper.initForProcess("android");
							loadedPackagesInProcess.add("android");
							...
							// Huawei
							try {
								fahm("com.android.server.pm.HwPackageManagerService", cl, "isOdexMode", X_MethodReplacement.returnConstant(false));
							} catch (XPHelpers.ClassNotFoundError | NoSuchMethodError ignored) {}

							try {
								String className = "com.android.server.pm." + (Build.VERSION.SDK_INT >= 23 ? "PackageDexOptimizer" : "PackageManagerService");
								fahm(className, cl, "dexEntryExists", String.class, X_MethodReplacement.returnConstant(true));
							} catch (XPHelpers.ClassNotFoundError | NoSuchMethodError ignored) {}
						}
					});
				}
		...
		fahm("android.app.ApplicationPackageManager", null, "getResourcesForApplication",
				ApplicationInfo.class, new X_MH() {
					@Override
					protected void bhm(mhp param) throws Throwable {
		...
		if (findFieldIfExists(ZygoteInit.class, "BOOT_START_TIME") != null) {
			setStaticLongField(ZygoteInit.class, "BOOT_START_TIME", XPB.BOOT_START_TIME);
		}
	 ...
	 * @throws Throwable
	 */
	/*package*/ static void hr() throws Throwable {
		...
		if (Build.VERSION.SDK_INT >= 24) {
			ham(classGTLR, "getOrCreateResources", new X_MH() {
				@Override
				protected void ahm(mhp param) throws Throwable {
			...
			ham(classGTLR, "getTopLevelResources", new X_MH() {
				@Override
				protected void bhm(mhp param) throws Throwable {
			...
			if (Build.VERSION.SDK_INT >= 19) {
				...
				ham(classGTLR, "getTopLevelThemedResources", new X_MH() {
					@Override
					protected void ahm(mhp param) throws Throwable {
		...
		if (Build.VERSION.SDK_INT >= 24) {
			...
			XPB.invalidateCallersNative(methods.toArray(new Member[methods.size()]));
		}

		// Replace TypedArrays with XTypedArrays
		hac(TypedArray.class, new X_MH() {
			@Override
			protected void ahm(mhp param) throws Throwable {
				TypedArray typedArray = (TypedArray) param.thisObject;
				Resources res = typedArray.getResources();
				if (res instanceof XResources) {
					XPB.setObjectClass(typedArray, XResources.XTypedArray.class);
				}
			}
		});

		// Replace system resources
		XResources systemRes = (XResources) XPB.cloneToSubclass(Resources.getSystem(), XResources.class);
	...
	private static XResources cloneToXResources(X_MH.mhp param, String resDir) {
		...
		XResources newRes = (XResources) XPB.cloneToSubclass(result, XResources.class);
		newRes.initObject(resDir);

		// Invoke handleInitPackageResources().
		if (newRes.isFirstLoad()) {
			String packageName = newRes.getPackageName();
			X_InitPackageResources.InitPackageResourcesParam resparam = new X_InitPackageResources.InitPackageResourcesParam(XPB.sInitPackageResourcesCallbacks);
	 ...
	 * @param classLoader
	 */
	private static void hxpi(ClassLoader classLoader) {
		try {
			fahm(INSTALLER_PACKAGE_NAME + ".XposedApp", classLoader, "getActiveXposedVersion",
					X_MethodReplacement.returnConstant(XPB.getXposedVersion()));

			fahm(INSTALLER_PACKAGE_NAME + ".XposedApp", classLoader, "onCreate", new X_MH() {
				@Override
				protected void ahm(mhp param) throws Throwable {
	...
	/*package*/ static void loadModules() throws IOException {
		...
		ClassLoader topClassLoader = XPB.BOOTCLASSLOADER;
		ClassLoader parent;
	...
	private static void loadModule(String apk, ClassLoader topClassLoader) {
		...
		if (dexFile.loadClass(XPB.class.getName(), topClassLoader) != null) {
		...
		ClassLoader mcl = new PathClassLoader(apk, XPB.BOOTCLASSLOADER);
		...
		try {
			...
			while ((moduleClassName = moduleClassesReader.readLine()) != null) {
				...
				try {
					...
					if (!IXPMod.class.isAssignableFrom(moduleClass)) {
						Log.e(TAG, "    This class doesn't implement any sub-interface of IXPMod, skipping it");
						continue;
					} else if (disableResources && IXPHInitPackageResources.class.isAssignableFrom(moduleClass)) {
					...
					if (XPB.isZygote) {
						if (moduleInstance instanceof IXPHZygoteInit) {
							IXPHZygoteInit.StartupParam param = new IXPHZygoteInit.StartupParam();
							param.modulePath = apk;
							param.startsSystemServer = startsSystemServer;
							((IXPHZygoteInit) moduleInstance).initZygote(param);
						}

						if (moduleInstance instanceof IXPHLoadPackage)
							XPB.hlp(new IXPHLoadPackage.Wrapper((IXPHLoadPackage) moduleInstance));

						if (moduleInstance instanceof IXPHInitPackageResources)
							XPB.hipr(new IXPHInitPackageResources.Wrapper((IXPHInitPackageResources) moduleInstance));
					} else {
						if (moduleInstance instanceof IXPHCmdInit) {
							IXPHCmdInit.StartupParam param = new IXPHCmdInit.StartupParam();
							param.modulePath = apk;
							param.startClassName = startClassName;
							((IXPHCmdInit) moduleInstance).initCmdApp(param);
						}
					}
				} catch (Throwable t) {
